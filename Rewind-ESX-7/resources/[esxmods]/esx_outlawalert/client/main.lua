...
AddEventHandler('esx_outlawalert:houseRobbery', function(targetCoords)
	if isPlayerWhitelisted and Config.GunshotAlert then
		local alpha = 250
		local houseblip = AddBlipForRadius(targetCoords.x, targetCoords.y, targetCoords.z, Config.BlipHouseRadius)

		SetBlipHighDetail(houseblip, true)
		SetBlipColour(houseblip, 1)
		SetBlipAlpha(houseblip, alpha)
...
RegisterNetEvent('police:houseRobbery')
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedTryingToEnterALockedVehicle(playerPed) or IsPedJacking(playerPed) then
			...
			if vehicle and Config.ShowCopsMisbehave or not isPlayerWhitelisted then
		...
		elseif IsPedInMeleeCombat(playerPed) then
			...
			if Config.ShowCopsMisbehave or not isPlayerWhitelisted then
				DecorSetInt(playerPed, 'isOutlaw', 2)
