...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsPedInAnyVehicle(player, false) and GetIsVehicleEngineRunning(vehicle) then
			if GetLastInputMethod (2) and IsControlJustReleased(2, Keys[Config.vehicle.keys.seatbelt]) and (has_value(vehiclesCars, vehicleClass) == true and vehicleClass ~= 8) then
				seatbeltIsOn = not seatbeltIsOn
			end
		end

		-- Vehicle Cruiser
		if GetLastInputMethod (2) and IsControlJustPressed(2, Keys[Config.vehicle.keys.cruiser]) and GetPedInVehicleSeat(vehicle, -1) == player and (has_value(vehiclesCars, vehicleClass) == true) then
