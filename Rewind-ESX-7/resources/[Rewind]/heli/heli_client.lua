...
-- Script starts here
local helicam = false
local polmav_hash = GetHashKey("supervolito")
	...
	while true do
		...
		if IsPlayerInPolmav() then
			...
			if IsHeliHighEnough(heli) then
				if GetLastInputMethod(2) and IsControlJustPressed(2, toggle_helicam) then -- Toggle Helicam
					PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
					helicam = true
				end
				
				if GetLastInputMethod(2) and IsControlJustPressed(2, toggle_rappel) then -- Initiate rappel
		...
		if helicam then
			...
			while helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) and IsHeliHighEnough(heli) do
				...
				if GetLastInputMethod(2) and IsControlJustPressed(2, toggle_vision) then
					PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
					ChangeVision()
				end
