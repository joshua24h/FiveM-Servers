...
function showFixePhoneHelper (coords)
    for number, data in pairs(FixePhone) do
        ...
        if dist <= 2.0 then
            ...
            if GetLastInputMethod( 2 ) and IsControlJustPressed(2, KeyTakeCall) then
...
--====================================================================================
 Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if GetLastInputMethod( 2 ) and  IsControlJustPressed(2, KeyOpenClose)  then
            TooglePhone()
        end
        if menuIsOpen == true then
            for _, value in ipairs(KeyToucheCloseEvent) do
                if IsControlJustPressed(2, value.code) then
        ...
        if IsControlJustPressed(1, KeyOpenClose) and GetLastInputMethod( 0 ) then
            ESX.TriggerServerCallback('gcphone:getItemAmount', function(qtty)
                if qtty > 0 then
                    ...
                    TriggerEvent('esx:showNotification', "Du besitzt kein ~r~Telefon~s~.")
                end
            end, 'phone')
        end
        if menuIsOpen == true then
